...
if __name__ == '__main__':
    ...
    # Load floor plans
    all_u0 = cuda.pinned_array((N, 514, 514))
    all_interior_mask = cuda.pinned_array((N, 512, 512), dtype='bool')
    all_u = cuda.pinned_array((N, 514, 514))
    ...
    # Make sure kernel gets JIT compiled
    dummy_u0 = cuda.pinned_array((514,514))
    dummy_u = cuda.pinned_array((514,514))
    dummy_mask = cuda.pinned_array((512, 512))
    ...
    for i, (u0, interior_mask) in enumerate(zip(all_u0, all_interior_mask)):
        final_u = jacobi_helper(u0, interior_mask, MAX_ITER)
        all_u[i] = final_u
